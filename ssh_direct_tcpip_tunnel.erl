...
restconf_test(LocalIp, LocalPort, RemoteIp, RemotePort, SshHost, Options) ->
    ...
    {ok, ChannelId} = direct_tcpip_tunnel(ConnRef,
                                          list_to_binary(LocalIp),
                                          LocalPort,  % Originator address
                                          list_to_binary(RemoteIp),
                                          RemotePort, % Destination address
                                          infinity),  % Timeout

    %% Send data through the tunnel
    DestHost = RemoteIp++":"++integer_to_list(RemotePort),
    Msg = "GET /restconf/data HTTP/1.1\r\n"
        "Host: "++DestHost++"\r\n"
...
tloop(ConnRef, ChannelId) ->
    receive
        ...
        Other ->            io:format("Other message: ~p~n", [Other]),
            tloop(ConnRef, ChannelId)
    after 10000 ->
...
direct_tcpip_tunnel(ConnectionRef,
                    OriginatorIP,
                    OriginatorPort,
                    DestHost,
                    DestPort,
                    Timeout)
  when is_binary(OriginatorIP) andalso is_binary(DestHost) ->
    ...
    ssh_connection:open_channel(ConnectionRef,
                                "direct-tcpip",
                                <<(byte_size(DestHost)):32, DestHost/binary,
                                  DestPort:32,
                                  (byte_size(OriginatorIP)):32, OriginatorIP/binary,
                                  OriginatorPort:32>>,
                                Timeout).
